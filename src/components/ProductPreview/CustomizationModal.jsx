...
export const CustomizationModal = ({
  ...
  const calculateTotalPrice = () => {
    const addOnPrice = selectedOptions.reduce((total, optionId) => {
      const option = customizationOptions.find((opt) => opt.item_id === optionId);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
      <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full h-[95vh] flex flex-col">
        ...
        <div className="flex items-center justify-between flex-shrink-0 p-4 border-b border-gray-200">
          ...
          <button onClick={onClose} className="p-2 transition duration-200 rounded-full hover:bg-gray-100">
          ...
          <div className="pt-2 pl-6 bg-gradient-to-r from-gray-50 to-white">
            <div className="flex items-center space-x-10">
              <div className="relative">
                <img src={jewelry.image_url1} alt={jewelry.name} className="object-cover w-32 h-32 shadow-lg rounded-xl" />
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center space-x-4">
                  <span className="text-2xl font-bold text-teal-600">LKR {jewelry.price.toLocaleString()}</span>
                  <span className="px-3 py-1 text-sm text-gray-500 bg-gray-100 rounded-full">Base Price</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex-shrink-0 p-4 border-t border-gray-200 bg-gray-50">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-[#1b4965]">LKR {calculateTotalPrice().toLocaleString()}</p>
            </div>
